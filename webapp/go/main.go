...
func getUserID(r *http.Request) (userID int64, errCode int, errMsg string) {
	...
	return uid.(int64), http.StatusOK, ""
}
...
func getCategoryByID(q sqlx.Queryer, categoryID int) (category Category, err error) {
	return mCategories[categoryID], nil
}
...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	user, errCode, errMsg := getUserID(r)
	if errMsg != "" {
	...
	for _, item := range items {
		...
		if transactionEvidence.ID > 0 {
			...
			ssr, err := APIShipmentStatus(r, getShipmentServiceURL(), &APIShipmentStatusReq{
...
func postItemEdit(w http.ResponseWriter, r *http.Request) {
	...
	seller, errCode, errMsg := getUserID(r)
	if errMsg != "" {
		outputErrorMsg(w, errCode, errMsg)
		return
	}
...
func getQRCode(w http.ResponseWriter, r *http.Request) {
	...
	if transactionEvidence.SellerID != seller {
...
func postBuy(w http.ResponseWriter, r *http.Request) {
	...
	scr, err := APIShipmentCreate(r, getShipmentServiceURL(), &APIShipmentCreateReq{
	...
	pstr, err := APIPaymentToken(r, getPaymentServiceURL(), &APIPaymentServiceTokenReq{
...
func postShip(w http.ResponseWriter, r *http.Request) {
	...
	img, err := APIShipmentRequest(r, getShipmentServiceURL(), &APIShipmentRequestReq{
		ReserveID: shipping.ReserveID,
	})
	if err != nil {
		log.Print(err)
...
func postComplete(w http.ResponseWriter, r *http.Request) {
	...
	buyer, errCode, errMsg := getUserID(r)
	if errMsg != "" {
		outputErrorMsg(w, errCode, errMsg)
		return
	}
	...
	if transactionEvidence.BuyerID != buyer {
		outputErrorMsg(w, http.StatusForbidden, "権限がありません")
		return
	}
...
func postSell(w http.ResponseWriter, r *http.Request) {
	...
	seller := User{}
	err = tx.Get(&seller, "SELECT * FROM `users` WHERE `id` = ? FOR UPDATE", user)
	if err == sql.ErrNoRows {
		outputErrorMsg(w, http.StatusNotFound, "user not found")
		tx.Rollback()
		return
